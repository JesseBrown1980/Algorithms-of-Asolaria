...
def get_args():
    ...
    p.add_argument('--hrm', action='store_true', help='two-timescale HRM: slow H module coupled to fast spheres')
...
class SphereMoE(nn.Module):
    def __init__(self, a):
        ...
        # HRM: slow planner updated once per refinement loop, guiding the fast spheres
        self.hcell = nn.GRUCell(a.dim, a.dim) if a.hrm else None
    def forward(self, idx):               # idx: (B, T) int64
        x = self.emb(idx)
        B, T, D = x.shape[0], x.shape[1], x.shape[2]
        h = torch.zeros(B * T, D, device=x.device) if self.hcell is not None else None
        for _ in range(max(1, self.a.loops)):     # rule-of-three refinement
            xin = x + h.view(B, T, D) if h is not None else x         # slow H guides fast L
            outs = torch.stack([s(xin) for s in self.spheres], dim=-2)  # (B,T,E,dim)
            if self.a.no_router:
                mix = outs.mean(dim=-2)
            else:
                gate = F.softmax(self.mirror(xin), dim=-1)            # (B,T,E)
                mix = (outs * gate.unsqueeze(-1)).sum(dim=-2)
            x = self.norm(x + mix)                                    # residual refine
            if h is not None:
                h = self.hcell(mix.reshape(B * T, D), h)              # H updates ONCE per loop (slow timescale)
...
def main():
    ...
    print(f"RESULT sphere-moe-hrm {cfg} params={nparams} data={a.bytes} val_bpc={bpc:.4f} seed={a.seed}")
