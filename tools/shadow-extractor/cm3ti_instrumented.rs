...
struct Model {
    ...
    rate: [i32; 256],
    w: Vec<i32>,      // fixed-point <<16 mixer weights
...
impl Model {
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        let mut dot: i64 = 0;
        for i in 0..self.nin { dot += self.w[self.wrow + i] as i64 * self.st[i] as i64; }
        let d = (dot >> 16) as i32;
...
fn compress_cost(data: &[u8], k: usize, costpath: &str) -> Vec<u8> {
    let mut m = Model::new(256, k); let mut enc = Enc::new(); let b = m.b;
    let mut costs: Vec<u16> = Vec::with_capacity(data.len());
    for &byte in data {
        ...
        for j in 0..b {
            ...
            let pb = if bit == 1 { p as f64 / 65536.0 } else { 1.0 - p as f64 / 65536.0 };
            bc += -(pb.max(1e-12)).log2();
            enc.encode(bit, p); m.update_bit(bit); c = (c << 1) | bit as usize;
        }
        // centibits, clamped to u16
        let cb = (bc * 100.0).round();
        costs.push(if cb > 65535.0 { 65535 } else { cb as u16 });
        m.update_symbol(s);
    }
    let mut out = Vec::with_capacity(costs.len() * 2);
    for c in &costs { out.extend_from_slice(&c.to_le_bytes()); }
    std::fs::write(costpath, &out).unwrap();
    enc.flush()
}
...
fn main() {
    ...
    let costmap = args.iter().position(|a| a == "--costmap").map(|i| args[i+1].clone());
    let t0 = Instant::now();
    let comp = if let Some(cp) = &costmap { compress_cost(&data, k, cp) } else { compress(&data, k) };
    ...
    let src_b = fs::metadata("cm3ti_instrumented.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-instr k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
