...
impl Model {
    ...
    fn begin_symbol(&mut self) {
        ...
        let cl=cls6(lb);
        let b1=if cls6(pb2)==cl {1usize} else {0};
        let b2=if cls6(pb3)==cl {1usize} else {0};
        self.sect=[cl, cl*2+b1, cl*4+b1*2+b2];   // 6 / 12(=V2) / 24 run-boundary levels
...
fn main() {
    ...
    let src_b = fs::metadata("cm3ti_stackrb.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-stackrb k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
