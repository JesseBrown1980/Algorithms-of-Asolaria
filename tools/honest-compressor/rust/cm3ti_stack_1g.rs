...
struct Model {
    ...
    rate: [i32; 256],
    w: Vec<i32>,      // base mixer: (bit,last-byte)
    ws: Vec<Vec<i32>>,   // stacked sector cubes at granularities 6,12,24
    srow: [usize;3], sect: [usize;3],
    apm: Vec<u16>,    // 1024*33, prob*16 fixed (0..4095)
    apm2: Vec<u16>,
    hist: Vec<u8>,
    mpos: Vec<u32>,  // bounded match: 2^24 slots, pos stored +1 (0=empty)
...
impl Model {
    fn new(v: usize, k: usize) -> Model {
        ...
        Model {
            ...
            ws: [6usize,12,24].iter().map(|&ns| vec![(0.2*65536.0) as i32; (b as usize*ns)*nin]).collect(),
            srow: [0;3], sect: [0;3],
            apm, apm2,
            hist: Vec::new(), mpos: vec![0u32; 1<<24],
    ...
    fn begin_symbol(&mut self) {
        ...
        if self.match_ptr < 0 && n >= MO {
            ...
            let p = self.mpos[(key & 0xFFFFFF) as usize] as usize;
            if p > 0 && p < n { self.match_ptr = p as i64; self.match_len = MO; }
        }
        ...
        let case_bit=if (65..=90).contains(&lb){1usize}else{0};
        let fine=cls6(lb)*8 + ((((cls6(pb2)&3)<<1)|case_bit)&7);  // 0..47
        self.sect=[fine>>3, fine>>2, fine>>1];  // 6-way, 12-way, 24-way
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        let ns=[6usize,12,24];
        for m in 0..3 { self.srow[m]=(j as usize*ns[m]+self.sect[m])*self.nin; }
        let mut dot: i64 = 0;
        for i in 0..self.nin {
            let mut wsum = self.w[self.wrow + i] as i64;
            for m in 0..3 { wsum += self.ws[m][self.srow[m]+i] as i64; }
            dot += wsum * self.st[i] as i64;
        }
        let d = (dot >> 18) as i32;  // /4 for 4 summed mixers
    ...
    fn update_bit(&mut self, bit: i32) {
        ...
        for i in 0..self.nin {
            ...
            for m in 0..3 { self.ws[m][self.srow[m]+i] += (self.st[i] * err) >> 14; }
        }
        for o in 1..=k {
    ...
    fn update_symbol(&mut self, s: i32) {
        ...
        if n >= MO {
            let mut key: u64 = 0;
            for &g in &self.hist[n - MO..] { key = key.wrapping_mul(0x01000193).wrapping_add(g as u64 + 1) & 0xFFFFFFFF; }
            self.mpos[(key & 0xFFFFFF) as usize] = n as u32;
        }
    }
}
...
fn main() {
    ...
    let src_b = fs::metadata("cm3ti_stack_1g.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-stack1g k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
