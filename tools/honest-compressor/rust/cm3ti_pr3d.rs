// cm3ti_pr3d.rs — causal PRIME_RULE_OF_THREE_DUALITY experiment built on
// cm3ti-sector. The measured sector winner remains unchanged in its own file.
// Every codec operation is integer deterministic and decoder reproducible.
//
// For N=3m predictor coordinates, select 2/3 in prime-addressed triads and
// evaluate NORMAL/inverse signed ports: U(N)=2*(2N/3)=4N/3. The P3 mode uses
// one RGB-style predictor triad (4 ports); P6 uses two triads (8 ports) with a
// shared 2D Omega mixer. This measures the HEX_Q15 opposed-marker chart;
// the full procedural 3D sphere binding remains unverified.
// Original cm3ti16 description: INTEGER-deterministic cm3t at 16-bit
// probability (tuned from cm3ti). Submission-grade: every arithmetic op is
...
const DECODER_SOURCE_BYTES: u64 = include_bytes!("cm3ti_pr3d.rs").len() as u64;
const OMEGA_W_LIMIT: i32 = 1 << 17;

// cos/sin(k*pi/3), Q15. This is the complete procedural 2D chart codebook.
...
// Prime addresses for the two predictor triads used by the N=6 cell.
const TRIAD_PRIMES: [usize; 2] = [2, 3];

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum OmegaMode { Off, P3, Rr3, P6 }
...
impl OmegaMode {
    ...
    fn label(self) -> &'static str {
        match self { Self::Off => "off", Self::P3 => "p3", Self::Rr3 => "rr3", Self::P6 => "p6" }
    }
}
...
fn omega_project(colors: [i32; 3], slice_id: usize, triad: usize, prime_select: bool) -> OmegaProjection {
    let prime = TRIAD_PRIMES[triad];
    let omit = if prime_select { (prime + slice_id) % 3 } else { (triad + slice_id) % 3 };
    let mut selected = [0usize; 2];
    let mut si = 0usize;
    for i in 0..3 { if i != omit { selected[si] = i; si += 1; } }

    let sum = colors[0] + colors[1] + colors[2];
    let mu = sum.div_euclid(3);
    let parity = sum.rem_euclid(3);
    let q = [colors[0] - mu, colors[1] - mu, colors[2] - mu];
    let qa = q[selected[0]];
    let qb = q[selected[1]];
    let qo = parity - qa - qb; // exact omitted coordinate, not a discarded residual
    ...
    for r in 0..4 {
        accum[0] += ports[r] as i64 * HEX_Q15[markers[r]].0 as i64;
        accum[1] += ports[r] as i64 * HEX_Q15[markers[r]].1 as i64;
    }
    let xy = [(accum[0] / 65536) as i32, (accum[1] / 65536) as i32];
    OmegaProjection { ports, markers, xy }
}
...
struct Model {
    ...
    wrow2: usize, sector: usize,
    omega_mode: OmegaMode,
    omega_w: Vec<i32>, // one shared (x,y) field per (bit,sector)
    omega_xy: [i32; 2],
    omega_row: usize,
...
impl Model {
    fn new(v: usize, k: usize, omega_mode: OmegaMode) -> Model {
        ...
        Model {
            ...
            rate,
            w: vec![19_660; (b as usize * 256) * nin],
            w2: vec![19_660; (b as usize * 6) * nin],
            wrow2: 0, sector: 0,
            omega_mode,
            omega_w: vec![0; (b as usize * 6) * 2],
            omega_xy: [0, 0],
            omega_row: 0,
            apm, apm2,
    ...
    fn begin_symbol(&mut self) {
        ...
        self.sector = if (65..=90).contains(&lb)||(97..=122).contains(&lb) {0}
            else if (48..=57).contains(&lb) {1}
            else if lb==32||lb==10||lb==9 {2}
            else if lb==60||lb==62||lb==38||lb==91||lb==93||lb==123||lb==125||lb==124||lb==61||lb==34 {3}
            else if lb>=128 {4} else {5};
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        // PRIME_RULE_OF_THREE_DUALITY feature construction. All inputs above
        // depend only on fixed model state and the already decoded prefix.
        self.omega_xy = [0, 0];
        if self.omega_mode != OmegaMode::Off {
            let mut context_sum = 0i64;
            for i in 0..=k { context_sum += self.st[i] as i64; }
            let context_mean = (context_sum / (k as i64 + 1)) as i32;
            let triad0 = [
                context_mean,
                self.st[k + 1],
                (self.st[k + 2] + self.st[k + 3]).div_euclid(2),
            ];
            let slice_id = self.hist.len() / SLICE_BYTES;
            let prime_select = self.omega_mode != OmegaMode::Rr3;
            let p0 = omega_project(triad0, slice_id, 0, prime_select);
            self.omega_xy = p0.xy;

            if self.omega_mode == OmegaMode::P6 {
                let deepest = if k > 0 { self.st[k - 1] } else { self.st[k] };
                let triad1 = [
                    self.st[0],
                    deepest,
                    (self.st[k + 2] - self.st[k + 3]).div_euclid(2),
                ];
                let p1 = omega_project(triad1, slice_id, 1, true);
                self.omega_xy[0] = (self.omega_xy[0] + p1.xy[0]).div_euclid(2);
                self.omega_xy[1] = (self.omega_xy[1] + p1.xy[1]).div_euclid(2);
            }
        }

        // fixed-point mixer
        self.wrow = (j as usize * 256 + self.bucket_base) * self.nin;
        self.wrow2 = (j as usize * 6 + self.sector) * self.nin;
        self.omega_row = (j as usize * 6 + self.sector) * 2;
        let mut dot: i64 = 0;
        for i in 0..self.nin { dot += (self.w[self.wrow + i] as i64 + self.w2[self.wrow2 + i] as i64) * self.st[i] as i64; }
        let omega_dot = self.omega_w[self.omega_row] as i64 * self.omega_xy[0] as i64
            + self.omega_w[self.omega_row + 1] as i64 * self.omega_xy[1] as i64;
        let d = (dot >> 17) as i32 + (omega_dot >> 18) as i32;
    ...
    fn update_bit(&mut self, bit: i32) {
        ...
        for i in 0..self.nin {
            ...
            self.w2[self.wrow2 + i] += (self.st[i] * err) >> 14;
        }
        if self.omega_mode != OmegaMode::Off {
            for axis in 0..2 {
                let old = self.omega_w[self.omega_row + axis] as i64;
                let delta = (self.omega_xy[axis] as i64 * err as i64) >> 18;
                self.omega_w[self.omega_row + axis] =
                    (old + delta).clamp(-(OMEGA_W_LIMIT as i64), OMEGA_W_LIMIT as i64) as i32;
            }
        }
        for o in 1..=k {
...
fn compress(data: &[u8], k: usize, omega_mode: OmegaMode) -> Vec<u8> {
    let mut m = Model::new(256, k, omega_mode); let mut enc = Enc::new(); let b = m.b;
    for &byte in data {
...
fn decompress(comp: &[u8], n: usize, k: usize, omega_mode: OmegaMode) -> Vec<u8> {
    let mut m = Model::new(256, k, omega_mode); let mut dec = Dec::new(comp); let b = m.b;
...
fn parse_cli_mode(arg: Option<&str>) -> Result<OmegaMode, String> {
    match arg {
        None | Some("--emit") => Ok(OmegaMode::P6),
        Some(value) => OmegaMode::parse(value)
            .ok_or_else(|| format!("invalid_mode:{value}:expected=off,p3,rr3,p6")),
    }
}
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("usage: cm3ti_pr3d <file> <k> [off|p3|rr3|p6] [--emit out.bin]");
        std::process::exit(1);
    }
    let data = fs::read(&args[1]).unwrap(); let n = data.len(); let k: usize = args[2].parse().unwrap();
    let omega_mode = match parse_cli_mode(args.get(3).map(String::as_str)) {
        Ok(mode) => mode,
        Err(reason) => {
            eprintln!("cm3ti-pr3d status=HOLD reason={reason}");
            std::process::exit(2);
        }
    };
    let sectors = match omega_mode { OmegaMode::P6 => 6, OmegaMode::Off => 0, _ => 3 };
    let (selected, views) = if sectors == 0 { (0, 0) } else { duality_counts(sectors).unwrap() };
    let sha_in = sha256(&data);
    let t0 = Instant::now(); let comp = compress(&data, k, omega_mode); let enc_s = t0.elapsed().as_secs_f64();
    let t1 = Instant::now(); let body = decompress(&comp, n, k, omega_mode); let dec_s = t1.elapsed().as_secs_f64();
    let ok = sha256(&body) == sha_in;
    let comp_sha = sha256(&comp);
    if !ok {
        eprintln!("cm3ti-pr3d status=HOLD reason=restore_sha256_mismatch comp_sha={}", &comp_sha[..16]);
        std::process::exit(2);
    }
    if let Some(i) = args.iter().position(|s| s == "--emit") {
        let path = match args.get(i + 1) {
            Some(path) => path,
            None => {
                eprintln!("cm3ti-pr3d status=HOLD reason=missing_emit_path");
                std::process::exit(2);
            }
        };
        fs::write(path, &comp).unwrap();
    }
    let src_b = DECODER_SOURCE_BYTES;
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-pr3d mode={} sectors={} selected={} views={} k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
        omega_mode.label(), sectors, selected, views, k, n, payload, src_b, total,
        (total as f64*8.0)/n as f64, "OK", &comp_sha[..16], enc_s, dec_s);
}
...
mod tests {
    ...
    #[test]
    fn normal_inverse_ports_and_markers_are_collision_free() {
        for slice_id in 0..12 {
            for triad in 0..2 {
                let p = omega_project([1700, -350, 921], slice_id, triad, true);
                assert_eq!(p.ports[2], -p.ports[0]);
                assert_eq!(p.ports[3], -p.ports[1]);
                assert_eq!(p.markers[2], (p.markers[0] + 3) % 6);
                assert_eq!(p.markers[3], (p.markers[1] + 3) % 6);
                let mut markers = p.markers;
                markers.sort();
    ...
    #[test]
    fn chart_is_integer_deterministic() {
        let a = omega_project([-2047, 0, 2047], 37, 0, true);
        let b = omega_project([-2047, 0, 2047], 37, 0, true);
        assert_eq!(a, b);
    }

    #[test]
    fn all_modes_round_trip() {
        let data = b"<page>prime rule of three duality 123 RGB rgb</page>\n".repeat(8);
        for mode in [OmegaMode::Off, OmegaMode::P3, OmegaMode::Rr3, OmegaMode::P6] {
            let comp = compress(&data, 1, mode);
            assert_eq!(decompress(&comp, data.len(), 1, mode), data);
        }
    }

    #[test]
    fn invalid_modes_hold_instead_of_falling_back_to_p6() {
        assert_eq!(parse_cli_mode(None), Ok(OmegaMode::P6));
        assert_eq!(parse_cli_mode(Some("p3")), Ok(OmegaMode::P3));
        assert!(parse_cli_mode(Some("typo")).is_err());
    }
}
