...
impl Model {
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        let mut dot: i64 = 0;
        for i in 0..self.nin { dot += (8*self.w[self.wrow + i] as i64 + 7*self.w2[self.wrow2 + i] as i64 + self.w2[self.wrow2b + i] as i64) * self.st[i] as i64; }
        let d = (dot >> 20) as i32;  // /16: 8*w + 7*sector + 1*neighbor
    ...
    fn update_bit(&mut self, bit: i32) {
        ...
        for i in 0..self.nin {
            ...
            self.w2[self.wrow2 + i] += (7 * self.st[i] * err) >> 17;
            self.w2[self.wrow2b + i] += (self.st[i] * err) >> 17;
        }
        for o in 1..=k {
...
fn main() {
    ...
    let src_b = fs::metadata("v5_tight.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-rainbow12-tight k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
