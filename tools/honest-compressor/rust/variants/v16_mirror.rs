...
struct Model {
    ...
    a1: (usize, i32, i32), a2: (usize, i32, i32), a3: (usize, i32, i32),
...
impl Model {
    fn new(v: usize, k: usize) -> Model {
        ...
        for c in 0..1024 {
            for j in 0..33 {
                ...
                apm[c * 33 + j] = p; apm2[c * 33 + j] = p; apm3[c * 33 + j] = p;
            }
        }
        Model {
            k, b, nin, tb,
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        let p_s2 = (pa2 + p_s1 * 3) >> 2;
        let actx3 = (self.sector * 2 + if self.match_bit >= 0 {1} else {0}) & 1023;
        let (pa3, s3) = Model::apm_apply(&self.tb, &self.apm3, actx3, p_s2);
        self.a3 = (s3.0, s3.1, p_s2);
        let mut p = (pa3 + p_s2 * 3) >> 2;
...
fn main() {
    ...
    println!("cm3ti-mirror k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
