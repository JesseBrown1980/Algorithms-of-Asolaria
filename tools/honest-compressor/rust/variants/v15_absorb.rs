...
impl Model {
    ...
    fn update_bit(&mut self, bit: i32) {
        ...
        for i in 0..self.nin {
            ...
            self.w2[self.wrow2 + i] += (self.st[i] * err) >> ABSORB[self.sector >> 1];
            self.w2[self.wrow2b + i] += (self.st[i] * err) >> ABSORB[self.sector_b >> 1];
        }
        for o in 1..=k {
...
const ABSORB: [i32; 6] = [16, 14, 14, 15, 15, 15]; // letters slow, digits/ws fast, rest baseline
...
fn main() {
    ...
    println!("cm3ti-absorb k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
