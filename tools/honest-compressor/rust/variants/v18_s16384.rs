...
impl Model {
    fn new(v: usize, k: usize) -> Model {
        ...
        Model {
            ...
            w2: vec![(0.3 * 65536.0) as i32; (b as usize * 16384) * nin],
    ...
    fn begin_symbol(&mut self) {
        ...
        let b1 = cls(lb); let b2 = cls(lb2); let _ = b1;
        self.sector = ((lb & 0xFF) as usize)*64 + b2*8 + cls(lb3);
        self.sector_b = ((lb2 & 0xFF) as usize)*64 + cls(lb3)*8 + cls(lb4);
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        self.wrow2 = (j as usize * 16384 + self.sector) * self.nin;
        self.wrow2b = (j as usize * 16384 + self.sector_b) * self.nin;
        let mut dot: i64 = 0;
...
fn main() {
    ...
    println!("cm3ti-s16384 k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
