...
impl Model {
    fn new(v: usize, k: usize) -> Model {
        ...
        Model {
            ...
            w2: vec![(0.3 * 65536.0) as i32; (b as usize * 24) * nin],
    ...
    fn begin_symbol(&mut self) {
        ...
        self.sector = b1*4 + run1;
        self.sector_b = b2*4 + if run2>3 {3} else {run2};
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        self.wrow2 = (j as usize * 24 + self.sector) * self.nin;
        self.wrow2b = (j as usize * 24 + self.sector_b) * self.nin;
        let mut dot: i64 = 0;
...
fn main() {
    ...
    let src_b = fs::metadata("v8_r24.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-rainbow24-even k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
