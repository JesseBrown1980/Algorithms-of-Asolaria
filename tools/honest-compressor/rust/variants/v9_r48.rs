...
impl Model {
    fn new(v: usize, k: usize) -> Model {
        ...
        Model {
            ...
            w2: vec![(0.3 * 65536.0) as i32; (b as usize * 48) * nin],
    ...
    fn begin_symbol(&mut self) {
        ...
        let lb5 = if n>4 { self.hist[n-5] as i32 } else { 0 };
        let lb6 = if n>5 { self.hist[n-6] as i32 } else { 0 };
        let lb7 = if n>6 { self.hist[n-7] as i32 } else { 0 };
        let lb8 = if n>7 { self.hist[n-8] as i32 } else { 0 };
        let cs = [b1,b2,b3,b4,cls(lb5),cls(lb6),cls(lb7),cls(lb8)];
        let mut run1 = 0usize; while run1 < 7 && cs[run1+1]==b1 { run1 += 1; }
        let mut run2 = 0usize; while run2 < 7 && run2+2 < 8 && cs[run2+2]==b2 { run2 += 1; }
        self.sector = b1*8 + run1;
        self.sector_b = b2*8 + run2;
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        self.wrow2 = (j as usize * 48 + self.sector) * self.nin;
        self.wrow2b = (j as usize * 48 + self.sector_b) * self.nin;
        let mut dot: i64 = 0;
...
fn main() {
    ...
    let src_b = fs::metadata("v9_r48.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-rainbow48-even k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
