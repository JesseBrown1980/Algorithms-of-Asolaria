...
impl Model {
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        let mut dot: i64 = 0;
        for i in 0..self.nin { dot += (4*self.w[self.wrow + i] as i64 + self.w2[self.wrow2 + i] as i64 + 3*self.w2[self.wrow2b + i] as i64) * self.st[i] as i64; }
        let d = (dot >> 19) as i32;  // /8: 4*w + 1*own + 3*neighbor (past-even probe)
    ...
    fn update_bit(&mut self, bit: i32) {
        ...
        for i in 0..self.nin {
            ...
            self.w2[self.wrow2 + i] += (self.st[i] * err) >> 16;
            self.w2[self.wrow2b + i] += (3 * self.st[i] * err) >> 16;
        }
        for o in 1..=k {
...
fn main() {
    ...
    let src_b = fs::metadata("v7_nbr.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-rainbow12-nbr k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
