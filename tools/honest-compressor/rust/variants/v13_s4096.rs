...
impl Model {
    fn new(v: usize, k: usize) -> Model {
        ...
        Model {
            ...
            w2: vec![(0.3 * 65536.0) as i32; (b as usize * 4096) * nin],
    ...
    fn begin_symbol(&mut self) {
        ...
        self.sector = ((lb & 0xFF) as usize)*16 + b2*2 + if b2==b1 {1} else {0};
        self.sector_b = ((lb2 & 0xFF) as usize)*16 + cls(lb3)*2 + if cls(lb3)==b2 {1} else {0};
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        self.wrow2 = (j as usize * 4096 + self.sector) * self.nin;
        self.wrow2b = (j as usize * 4096 + self.sector_b) * self.nin;
        let mut dot: i64 = 0;
...
fn main() {
    ...
    println!("cm3ti-s4096 k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
