...
fn prime(m: &mut Model, prior: &[u8]) {
    let b = m.b;
    for &byte in prior {
        let s = byte as i32; m.begin_symbol(); let mut c = 1usize;
        for j in 0..b {
            let bit = (s >> (b - 1 - j)) & 1;
            let _p = m.predict_bit(c, j);
            m.update_bit(bit); c = (c << 1) | bit as usize;
        }
        m.update_symbol(s);
    }
}
fn compress(data: &[u8], k: usize, prior: &[u8]) -> Vec<u8> {
    let mut m = Model::new(256, k); prime(&mut m, prior); let mut enc = Enc::new(); let b = m.b;
    for &byte in data {
        let s = byte as i32; m.begin_symbol(); let mut c = 1usize;
        for j in 0..b {
            let bit = (s >> (b - 1 - j)) & 1;
...
fn decompress(comp: &[u8], n: usize, k: usize, prior: &[u8]) -> Vec<u8> {
    let mut m = Model::new(256, k); prime(&mut m, prior); let mut dec = Dec::new(comp); let b = m.b;
...
fn main() {
    ...
    let prior: Vec<u8> = if args.len() >= 4 && args[3] != "--emit" { fs::read(&args[3]).unwrap() } else { Vec::new() };
    let sha_in = sha256(&data);
    let t0 = Instant::now(); let comp = compress(&data, k, &prior); let enc_s = t0.elapsed().as_secs_f64();
    let t1 = Instant::now(); let body = decompress(&comp, n, k, &prior); let dec_s = t1.elapsed().as_secs_f64();
    let ok = sha256(&body) == sha_in;
    let comp_sha = sha256(&comp);
    if args.len() >= 5 && args[3] == "--emit" { fs::write(&args[4], &comp).unwrap(); }
    let src_b = fs::metadata("v14_primed.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b + prior.len() as u64;
    println!("cm3ti-primed prior={} k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
        prior.len(), k, n, payload, src_b, total, (total as f64*8.0)/n as f64, if ok {"OK"} else {"FAIL"}, &comp_sha[..16], enc_s, dec_s);
}
