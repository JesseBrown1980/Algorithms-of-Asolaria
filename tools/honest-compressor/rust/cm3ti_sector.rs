...
impl Model {
    fn new(v: usize, k: usize) -> Model {
        ...
        Model {
            ...
            w2: vec![(0.3 * 65536.0) as i32; (b as usize * 6) * nin],
            wrow2: 0, sector: 0,
            apm, apm2,
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        let d = (dot >> 17) as i32;  // /2 for the two summed mixers
...
fn main() {
    ...
    let src_b = fs::metadata("cm3ti_sector.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-sector k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
