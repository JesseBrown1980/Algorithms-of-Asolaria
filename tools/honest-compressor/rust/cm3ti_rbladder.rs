...
impl Model {
    fn new(v: usize, k: usize) -> Model {
        ...
        let mode = std::env::var("RB_MODE").unwrap_or_else(|_| "c3".into());
        // ladder config: level sector-counts, per-row weights (2 rows/level), base weight,
        // shift = 16 + log2(basew + sum(rowweights)) — totals kept at powers of two
        let (lvl_size, rowp, basew, sshift): (Vec<usize>, Vec<i64>, i64, u32) = match mode.as_str() {
            "12" => (vec![12], vec![2,2], 4, 19),          // champion regression anchor
            "24" => (vec![24], vec![2,2], 4, 19),          // separated fine
        ...
        let mut lvl_off = vec![0usize; lvl_size.len()];
        let mut acc = 0usize;
    ...
    fn begin_symbol(&mut self) {
        ...
        fn cls6(x:i32)->usize{ if (65..=90).contains(&x)||(97..=122).contains(&x){0} else if (48..=57).contains(&x){1} else if x==32||x==10||x==9{2} else if x==60||x==62||x==38||x==91||x==93||x==123||x==125||x==124||x==61||x==34{3} else if x>=128{4} else{5} }
        let pb2=if n>1{self.hist[n-2] as i32}else{0};
        ...
        for li in 0..self.lvl_size.len() {
            ...
            self.sects[ri] = self.lvl_off[li] + sb; ri += 1;
        }
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        let nr = self.rweight.len();
        for r in 0..nr { self.rows[r] = (j as usize * self.wtot + self.sects[r]) * self.nin; }
        let mut dot: i64 = 0;
        for i in 0..self.nin {
            let mut a: i64 = self.basew * (self.w[self.wrow + i] as i64);
            for r in 0..nr { a += self.rweight[r] * (self.w2[self.rows[r] + i] as i64); }
            dot += a * self.st[i] as i64;
        }
        let d = (dot >> self.sshift) as i32;  // total weight is a power of two per config
...
fn main() {
    ...
    let src_b = fs::metadata("cm3ti_rbladder.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    let mode = std::env::var("RB_MODE").unwrap_or_else(|_| "c3".into());
    println!("cm3ti-rbladder[{}] k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
        mode, k, n, payload, src_b, total, (total as f64*8.0)/n as f64, if ok {"OK"} else {"FAIL"}, &comp_sha[..16], enc_s, dec_s);
}
