...
impl Model {
    ...
    fn begin_symbol(&mut self) {
        ...
        let cl=cls6(lb); let cp=cls6(pb2);
        self.sector = cl*2 + if cp==cl {1} else {0};              // s1: 12-way run/boundary (V2)
        self.sector_b = cp*2 + if cls6(pb3)==cp {1} else {0};     // s2: neighbor color
    ...
    fn predict_bit(&mut self, c: usize, j: u32) -> i32 {
        ...
        let mut dot: i64 = 0;
        for i in 0..self.nin {
            dot += (4*(self.w[self.wrow + i] as i64) + 2*(self.w2[self.wrow2 + i] as i64) + 2*(self.w2[self.wrow2b + i] as i64)) * self.st[i] as i64;
        }
        let d = (dot >> 19) as i32;  // weights 4+2+2=8, >>16 base +>>3
...
fn main() {
    ...
    let src_b = fs::metadata("cm3ti_rainbow12.rs").map(|m| m.len()).unwrap_or(0);
    let payload = comp.len() as u64; let total = payload + src_b;
    println!("cm3ti-rainbow12 k={} N={} payload={} decoder_src={} total={} bpc_total={:.4} restore={} comp_sha={} enc={:.0}s dec={:.0}s",
