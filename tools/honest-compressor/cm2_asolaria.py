#!/usr/bin/env python3
# cm2: bitwise context-mixing compressor (PAQ/lpaq-lite) for the honest-compressor lane.
#
...
class Model:
    def __init__(self, V, k, match_order=5, lr=0.02):
        ...
        self.nin = k + 2                     # k context models + order0 + match
    ...
    def predict_bit(self, c, j):
        ...
        p1 = squash(dot)
        # APM/SSE stage: key on (order-1 ctx bucket, interpolated p bin)
        actx = ((self.ctx_h[1] ^ (c * 0x2545F491)) & 1023) if self.k >= 1 else (c & 1023)
        pf = min(max(p1, 1e-6), 1 - 1e-6)
        z = np.log(pf / (1 - pf))            # stretch
        b = (z + 8.0) * 2.0                  # 33 bins over [-8,8]
...
if __name__ == "__main__":
    ...
    for arg in sys.argv[2:]:
        ...
        r = run(path, nm, k)
        line = (f"cm2 merges={nm:5d} k={k}  V={r['V']:5d} glyphs={r['glyphs']:7d} "
        ...
        with open("cm2-sweep.log", "a") as f:
            f.write(line + "\n")
