...
# APM/SSE: refine mixed p by (context, quantized stretch). NCTX contexts x 33 knots.
APM_CTX = 1024
APM_N = 33
apm = array('H', [0]*(APM_CTX*APM_N))
for c in range(APM_CTX):
    for j in range(APM_N):
        x = (j-16)*128            # stretch domain
        apm[c*APM_N+j] = squash(x)*16    # store p*16 for resolution
def apm_pp(ctx, p):
    # p in 1..4095 -> stretch -> interpolate knots
    s = ST[p]                     # -~2047..2047
    s = 2047 if s>2047 else (-2048 if s<-2048 else s)
    pos = (s+2048)/128.0
    j = int(pos); w = pos-j
    if j >= APM_N-1: j = APM_N-2; w = 1.0
    base = ctx*APM_N+j
    lo = apm[base]; hi = apm[base+1]
    pr = int((lo*(1-w)+hi*w)/16)
    return (1 if pr<1 else (4095 if pr>4095 else pr)), base, w
def apm_update(base, w, bit):
    g = (bit<<12)*16
    k = base if w < 0.5 else base+1
    v = apm[k] + ((g - apm[k])>>7)
    apm[k] = 0 if v < 0 else (65535 if v > 65535 else v)
...
def run():
    ...
    for i in range(len(hist)):
        ...
        for bp in range(8):
            ...
            # APM/SSE refinement
            apm_ctx = (((hist[i-1] if i>0 else 0) & 0x7F)<<3) | bp
            p2, ab, aw = apm_pp(apm_ctx, p)
            pfin = (p + 3*p2) >> 2
            pfin = 1 if pfin<1 else (4095 if pfin>4095 else pfin)
            # actual bit
            bit = (hist[i] >> (7-bp)) & 1
            pf = pfin/4096.0
            loss += -math.log2(pf if bit else 1-pf)
            apm_update(ab, aw, bit)
            # update mixer
            err = ((bit<<12) - p) / 4096.0
            g = LR*err
            for m in range(NM):
